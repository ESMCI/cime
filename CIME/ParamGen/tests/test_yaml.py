...
class TestYamlConstructor(unittest.TestCase):
    ...
    def test_mom_input(self):
        ...
        mom_input = ParamGen.from_yaml('yaml/MOM_input.yaml')
        mom_input.reduce(lambda varname: case.get_value(varname)) 
        self.assertEqual(mom_input.data,
            {'Global':
                {'INPUTDIR': {'value': '/foo/inputdata/ocn/mom/tx0.66v1'},
                'RESTORE_SALINITY': {'value': True},
                'INIT_LAYERS_FROM_Z_FILE': {'value': True},
                'TEMP_SALT_Z_INIT_FILE': {'value': "woa18_04_initial_conditions.nc"}
                }
            }
        )
    ...
    def test_input_data_list(self):
        ...
        def input_data_list_expand_func(varname):
            ...
            return val
        input_data_list = ParamGen.from_yaml('yaml/mom.input_data_list.yaml')
        input_data_list.reduce(input_data_list_expand_func)
        self.assertEqual(input_data_list.data,
            {'mom.input_data_list':
