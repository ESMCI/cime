...
class DAE(SystemTestsCompareTwo):
    ...
    def _case_two_setup(self):
        ...
        if len(self._case.get_value("DATA_ASSIMILATION_SCRIPT")) == 0:
            ...
            da_dir = os.path.join(
                self._case.get_value("CIMEROOT"), "scripts/data_assimilation"
            )
            expect(
            ...
            expect(
                os.path.isfile(da_file),
                "ERROR: da_file, '{}', does not exist".format(da_file),
            )
        ...
        expect(
            (stopn % da_cycles) == 0,
            "ERROR: DAE test with {0} cycles requires that STOP_N be divisible by {0}".format(
                da_cycles
            ),
        )
    ...
    def run_phase(self):  # pylint: disable=arguments-differ
        ...
        expect(
            ...
            "ERROR: There were {:d} DA cycles in run but {:d} DA files were found".format(
        ...
        for fname in da_files:
            ...
            with gzip.open(fname, "r") as dfile:
                for bline in dfile:
                    ...
                    if "caseroot" in line[0:8]:
                        ...
                        expect(
                            ...
                            "ERROR: Wrong cycle ({:d}) found in {} (expected {:d})".format(
                        ...
                        expect(
                            False,
                            "ERROR: Unrecognized line ('{}') found in {}".format(
                                line, fname
                            ),
                        )
                ...
                expect(
                    ...
                    "ERROR: Expected {} post-DA resume signal message(s), {} found in {}".format(
                        expected_signal, found_signal, fname
                    ),
                )
                expect(
                    found_init == expected_init,
                    "ERROR: Expected {} Initial run message(s), {} found in {}".format(
                        expected_init, found_init, fname
                    ),
                )
            # End with
