...
class ERIO(SystemTestsCommon):
    ...
    def _full_run(self, pio_type):
        ...
        expect(
            self._stop_n > 2, "ERROR: stop_n value {:d} too short".format(self._stop_n)
        )
        logger.info(
            "doing an {0} {1} initial test with restart file at {2} {1} with pio type {3}".format(
                str(self._stop_n), stop_option, str(rest_n), pio_type
            )
        )
    ...
    def _restart_run(self, pio_type, other_pio_type):
        ...
        self._case.flush()
        logger.info(
            "doing an {} {} restart test with {} against {}".format(
