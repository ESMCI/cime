...
def parse_command_line(args):
    ...
    parser.add_argument("--tagname", help="Name of the tag being tested.")
    parser.add_argument("--testid", help="Test id, ie c2_0_a6g_ing,c2_0_b6g_gnu.")
    parser.add_argument(
        "--testroot", help="Root directory for tests to populate the database."
    )
    parser.add_argument("--testtype", help="Type of test, prealpha or prebeta.")
    parser.add_argument(
        "--dryrun",
        action="store_true",
    ...
    parser.add_argument(
        "--dumpxml", action="store_true", help="Dump XML test results to sceen."
    )
...
def get_testreporter_xml(testroot, testid, tagname, testtype):
    ...
    expect(
        len(xml_file) > 0,
        "Tests not found.  It's possible your testid, {} is wrong.".format(testid),
    )
    ...
    testxml.setup_header(
        tagname, machine, compiler, mpilib, testroot, testtype, baseline
    )
