...
class TestSpec(GenericXML):
    ...
    def set_header(
        self, testroot, machine, testid, baselinetag=None, baselineroot=None
    ):
    ...
    def add_test(self, compiler, mpilib, testname):
        expect(
            testname not in self._testnodes,
            "Test {} already in testlist".format(testname),
        )

        telem = self.make_child(
            "test", attributes={"name": testname}, root=self._testlist_node
        )
