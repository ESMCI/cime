...
def rename_all_hist_files(case, suffix):
    ...
    for model in _iter_model_file_substrs(case):
        ...
        if model == "cpl":
            ...
            mname = model
        test_hists = archive.get_all_hist_files(
            case.get_value("CASE"), mname, rundir, ref_case=ref_case
        )
    ...
    expect(
        num_renamed > 0,
        "renaming failed: no hist files found in rundir '{}'".format(rundir),
    )
...
def _hists_match(model, hists1, hists2, suffix1="", suffix2=""):
    ...
    for hists, suffix, normalized, multi_normalized in [
        ...
        for hist in hists:
            ...
            expect(
                offset >= 0,
                "ERROR: cant find model name {} in {}".format(model, hist_basename),
            )
            normalized_name = os.path.basename(hist_basename[offset:])
            if suffix != "":
                expect(
                    normalized_name.endswith(suffix),
                    "How did '{}' not have suffix '{}'".format(hist, suffix),
                )
    ...
    match_ups = sorted(
        [
            (hists1[normalized1.index(item)], hists2[normalized2.index(item)])
            for item in both
        ]
    )
    ...
    if not multiinst:
        expect(
            len(match_ups) + len(set_of_1_not_2) == len(hists1), "Programming error1"
        )
        expect(
            len(match_ups) + len(set_of_2_not_1) == len(hists2), "Programming error2"
        )
...
def _compare_hists(
    ...
    comments = "Comparing hists for case '{}' dir1='{}', suffix1='{}',  dir2='{}' suffix2='{}'\n".format(
        casename, from_dir1, suffix1, from_dir2, suffix2
    )
    ...
    for model in _iter_model_file_substrs(case):
        ...
        hists1 = archive.get_latest_hist_files(
            casename, model, from_dir1, suffix=suffix1, ref_case=ref_case
        )
        hists2 = archive.get_latest_hist_files(
            casename, model, from_dir2, suffix=suffix2, ref_case=ref_case
        )
        ...
        one_not_two, two_not_one, match_ups = _hists_match(
            model, hists1, hists2, suffix1, suffix2
        )
        for item in one_not_two:
            if "initial" in item:
                continue
            comments += "    File '{}' {} in '{}' with suffix '{}'\n".format(
                item, NO_COMPARE, from_dir2, suffix2
            )
            all_success = False
        ...
        for hist1, hist2 in match_ups:
            ...
            if success:
                ...
                expected_log_file = os.path.join(
                    casedir, os.path.basename(cprnc_log_file)
                )
                if not (
                    os.path.exists(expected_log_file)
                    and filecmp.cmp(cprnc_log_file, expected_log_file)
                ):
                    try:
                        safe_copy(cprnc_log_file, casedir)
                    except (OSError, IOError) as _:
                        logger.warning(
                            "Could not copy {} to {}".format(cprnc_log_file, casedir)
                        )
...
def cprnc(
    ...
    if outfile_suffix is None:
        ...
        cpr_stat = run_cmd(
            "{} -m {} {}".format(cprnc_exe, file1, file2),
            combine_output=True,
...
def compare_baseline(case, baseline_dir=None, outfile_suffix=""):
    ...
    for bdir in dirs_to_check:
        if not os.path.isdir(bdir):
            return False, "ERROR {} baseline directory '{}' does not exist".format(
...
def _generate_baseline_impl(case, baseline_dir=None, allow_baseline_overwrite=False):
    ...
    newestcpllogfile = case.get_latest_cpl_log(
        coupler_log_path=case.get_value("RUNDIR"), cplname=cplname
    )
    if newestcpllogfile is None:
        logger.warning(
            "No {}.log file found in directory {}".format(
                cplname, case.get_value("RUNDIR")
            )
        )
    else:
        safe_copy(
            newestcpllogfile,
            os.path.join(basegen_dir, "{}.log.gz".format(cplname)),
            preserve_meta=False,
        )
