...
###############################################################################
    ...
    # directory).
    parser.add_argument("--user-mods-dirs", "--user-mods-dir", nargs='*',
    ...
    parser.add_argument("--ngpus-per-node", default=0, type=int,
    ...
    return args.case, args.compset, args.res, args.machine, args.compiler,\
        ...
        args.user_mods_dirs, args.pesfile, \
...
###############################################################################
    ...
    casename, compset, grid, machine, compiler, \
        ...
        user_mods_dirs, pesfile, \
        gridfile, srcroot, test, multi_driver, ninst, walltime, \
        queue, output_root, script_root, run_unsupported, \
        answer, input_dir, driver, \
        workflow, non_local, extra_machines_dir, case_group, ngpus_per_node = parse_command_line(sys.argv, cimeroot, description)
    ...
    if user_mods_dirs is not None:
        user_mods_dirs = [os.path.abspath(one_user_mods_dir) if os.path.isdir(one_user_mods_dir)
    ...
    with Case(caseroot, read_only=False) as case:
        # Configure the Case
        case.create(casename, srcroot, compset, grid, user_mods_dirs=user_mods_dirs,
