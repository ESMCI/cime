...
class TestUserModSupport(unittest.TestCase):
    ...
    def createUserMod(self, name, include_dirs=None):
        ...
        if include_dirs:
            with open(
                os.path.join(mod_dir, "include_user_mods"), "w"
            ) as include_user_mods:
                for one_include in include_dirs:
                    include_user_mods.write(
    ...
    def assertResults(
        ...
        self.assertTrue(
            os.path.isfile(path_to_user_nl_cpl),
        ...
        path_to_shell_commands_result = os.path.join(
            self._caseroot, "shell_commands_result"
        )
        self.assertTrue(
            os.path.isfile(path_to_shell_commands_result),
        ...
        self.assertTrue(
            os.path.isfile(path_to_sourcemod),
    ...
    def test_basic(self):
        ...
        apply_user_mods(self._caseroot, os.path.join(self._user_mods_parent_dir, "foo"))
        self.assertResults(
            expected_user_nl_cpl="foo\n",
    ...
    def test_keepexe(self):
        self.createUserMod("foo")
        with self.assertRaisesRegex(CIMEError, "cannot have any source mods"):
            apply_user_mods(
                self._caseroot,
    ...
    def test_two_applications(self):
        ...
        apply_user_mods(
            self._caseroot, os.path.join(self._user_mods_parent_dir, "foo1")
        )
        apply_user_mods(
            self._caseroot, os.path.join(self._user_mods_parent_dir, "foo2")
        )
        self.assertResults(
            expected_user_nl_cpl="foo1\nfoo2\n",
    ...
    def test_include(self):
        ...
        apply_user_mods(
            self._caseroot, os.path.join(self._user_mods_parent_dir, "derived")
        )

        self.assertResults(
            expected_user_nl_cpl="base\nderived\n",
