...
if sys.hexversion < 0x02070000:
    ...
    print(
        "It appears that you are running python {0}".format(
...
def commandline_options():
    ...
    parser.add_argument("--debug", action="store_true", help="extra debugging output")

    parser.add_argument(
        "--config", nargs=1, default=["tools_autodoc.cfg"], help="path to config file"
    )
...
def get_scripts(config, doc_dir):
    ...
    for f in scripts_files:
        ...
        with open(script_file, "w") as tf:
            contents = _script_template.substitute(
                script_name=f, scripts_dir=scripts_dir
            )
            tf.write(contents)
