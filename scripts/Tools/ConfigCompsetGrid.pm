...
sub getCompsetLongname
{
    ...
    my $parser = XML::LibXML->new( no_blanks => 1);
    my $xml_compsets = $parser->parse_file($compsets_file);
...
sub _setGridMaps
{
    ...
    @nodes = $xml_grids_maps->findnodes(".//gridmap[\@grid1=\"$atm_grid\" and \@ocn_grid=\"$ocn_grid\"]/ATM2OCN_FMAPNAME");
    ...
    @nodes = $xml_grids_maps->findnodes(".//gridmap[\@atm_rid=\"$atm_grid\" and \@ocn_grid=\"$ocn_grid\"]/ATM2OCN_VMAPNAME");
    ...
    @nodes = $xml_grids_maps->findnodes(".//gridmap[\@ocn_grid=\"$ocn_grid\" and \@rof_grid=\"$rof_grid\"]/ROF2LND_FMAPNAME");
...
sub _setComponent {
    ...
    my $parser = XML::LibXML->new( no_blanks => 1);
    my $xml = $parser->parse_file($component_file);

    # Parse the compnent file and add all the variable definitions to $cfg_ref
