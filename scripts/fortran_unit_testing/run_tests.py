...
runs "make clean".""",
    ...
    parser.add_argument(
        "--cmake-args", help="""Additional arguments to pass to CMake."""
    )
    parser.add_argument(
        "--comp-interface",
        default="mct",
    ...
    parser.add_argument(
        "--no-color", action="store_false", help="""Turn off colorized output."""
    )
    parser.add_argument(
        "--compiler",
...
requires genf90.pl to be in the user's path.""",
    ...
    parser.add_argument(
        "-T", "--ctest-args", help="""Additional arguments to pass to CTest."""
    )
    parser.add_argument(
        "--use-env-compiler",
        action="store_true",
...
def cmake_stage(
    ...
    if not os.path.isfile("CMakeCache.txt"):
        ...
        cmake_command = [
            ...
            + os.path.abspath(os.path.join(_CIMEROOT, "CIME", "data", "src", "CMake")),
        ...
        if enable_genf90:
            ...
            genf90_dir = os.path.join(_CIMEROOT, "CIME", "data", "externals", "genf90")
...
def _main():
    ...
    if clean:
        ...
        for file_ in pwd_contents:
            if (
                file_ in ("Macros.cmake", "env_mach_specific.xml")
    ...
    if "NETCDF_PATH" in os.environ and not "NETCDF" in os.environ:
        ...
        logger.info(
            "Setting NETCDF environment variable: {}".format(os.environ["NETCDF_PATH"])
        )
    ...
    if "NETCDFROOT" in os.environ and not "NETCDF" in os.environ:
        ...
        logger.info(
            "Setting NETCDF environment variable: {}".format(os.environ["NETCDFROOT"])
        )
    ...
    for spec in suite_specs:
        ...
        for label, directory in spec:
            ...
            output = run_cmd_no_fail(
                " ".join(ctest_command), from_dir=label, combine_output=True
            )
            logger.info(output)
