...
class GridFTP(GenericServer):
    ...
    def fileexists(self, rel_path):
        stat,out,err = run_cmd("globus-url-copy -list {}".format(os.path.join(self._root_address, os.path.dirname(rel_path))+os.sep), timeout=CIME.utils.SHORT_OP_TIMEOUT)
    ...
    def getfile(self, rel_path, full_path):
        stat, _,err = run_cmd("globus-url-copy -v {} file://{}".format(os.path.join(self._root_address, rel_path), full_path), timeout=CIME.utils.LONG_OP_TIMEOUT)
    ...
    def getdirectory(self, rel_path, full_path):
        stat, _,err = run_cmd("globus-url-copy -v -r {}{} file://{}{}".format(os.path.join(self._root_address, rel_path), os.sep, full_path, os.sep), timeout=CIME.utils.LONG_OP_TIMEOUT)
