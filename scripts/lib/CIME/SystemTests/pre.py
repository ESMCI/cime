    ...
    def run_phase(self):  # pylint: disable=arguments-differ
        ...
        for comp in comps:
            ...
            for index in range(1, ninst + 1):
                ...
                expect(
                    (len(restart_files_2) == 1),
                    "Missing case2 restart file, {}",
                    glob_str,
                )
                rfile2 = restart_files_2[0]
                ok = cprnc(
                    comp, rfile1, rfile2, self._case, rundir2, cprnc_exe=self._cprnc_exe
                )[0]
                logger.warning(
                    "CPRNC result for {}: {}".format(
                        os.path.basename(rfile1),
                        "PASS" if (ok == should_match) else "FAIL",
                    )
                )
