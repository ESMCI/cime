...
class Case(object):
    ...
    def configure(self, compset_name, grid_name, machine_name=None,
        ...
        #               more than one gpu per node available 
        compiler = self.get_value("COMPILER")
        if compiler in ["pgi-gpu", "nvhpc-gpu"]:
            expect(ngpus_per_node > 0," ngpus_per_node is expected > 0 for pgi-gpu and nvhpc-gpu compiler; current value is {}".format(ngpus_per_node))
