...
class BaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._compiler          = self.MACHINE.get_default_compiler() if self.TEST_COMPILER is None else self.TEST_COMPILER
    ...
    def assert_test_status(self, test_name, test_status_obj, test_phase, expected_stat):
        ...
        self.assertEqual(test_status, expected_stat, msg="Problem with {}: for phase '{}': has status '{}', expected '{}'".format(test_name, test_phase, test_status, expected_stat))

    def run_cmd_assert_result(self, cmd, from_dir=None, expected_stat=0, env=None, verbose=False):
        from_dir = os.getcwd() if from_dir is None else from_dir
        stat, output, errput = utils.run_cmd(cmd, from_dir=from_dir, env=env, verbose=verbose)
        if expected_stat == 0:
    ...
    def assert_dashboard_has_build(self, build_name, expected_count=1):
        ...
        if utils.get_model() == "e3sm":
            ...
            utils.run_cmd_no_fail("wget https://my.cdash.org/api/v1/index.php?project=ACME_test --no-check-certificate -O %s" % wget_file)
    ...
    def kill_subprocesses(self, name=None, sig=signal.SIGKILL, expected_num_killed=None):
        # Kill all subprocesses
        proc_ids = utils.find_proc_id(proc_name=name, children_only=True)
        if (expected_num_killed is not None):
            self.assertEqual(len(proc_ids), expected_num_killed,
    ...
    def _create_test(self, extra_args, test_id=None, run_errors=False, env_changes=""):
        ...
        test_id = "{}-{}".format(self._baseline_name, utils.get_timestamp()) if test_id is None else test_id
        extra_args.append("-t {}".format(test_id))
        extra_args.append("--baseline-root {}".format(self._baseline_area))
        if self.NO_BATCH:
            extra_args.append("--no-batch")
        if self.TEST_COMPILER and ([extra_arg for extra_arg in extra_args if "--compiler" in extra_arg] == []):
        ...
        output = self.run_cmd_assert_result("{} {}/create_test {}".format(env_changes, self.SCRIPT_DIR, " ".join(extra_args)),
    ...
    def get_casedir(self, case_fragment, all_cases):
        ...
        self.assertTrue(len(potential_matches) == 1, "Ambiguous casedir selection for {}, found  {}  among  {}".format(case_fragment, potential_matches, all_cases))
    ...
    def verify_perms(self, root_dir):
        for root, dirs, files in os.walk(root_dir):
            for filename in files:
                ...
                self.assertTrue(st.st_mode & stat.S_IWGRP, msg="file {} is not group writeable".format(full_path))
                self.assertTrue(st.st_mode & stat.S_IRGRP, msg="file {} is not group readable".format(full_path))
                self.assertTrue(st.st_mode & stat.S_IROTH, msg="file {} is not world readable".format(full_path))

            for dirname in dirs:
                full_path = os.path.join(root, dirname)
                st = os.stat(full_path)

                self.assertTrue(st.st_mode & stat.S_IWGRP, msg="dir {} is not group writable".format(full_path))
                self.assertTrue(st.st_mode & stat.S_IRGRP, msg="dir {} is not group readable".format(full_path))
                self.assertTrue(st.st_mode & stat.S_IXGRP, msg="dir {} is not group executable".format(full_path))
                self.assertTrue(st.st_mode & stat.S_IROTH, msg="dir {} is not world readable".format(full_path))
                self.assertTrue(st.st_mode & stat.S_IXOTH, msg="dir {} is not world executable".format(full_path))
