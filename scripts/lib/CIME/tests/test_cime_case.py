...
class TestCimeCase(base.BaseTestCase):
    def test_cime_case(self):
        ...
        self.assertTrue(type(self.MACHINE.get_value("PROJECT_REQUIRED")) in [type(None) , bool])
        ...
        with Case(casedir, read_only=False) as case:
            ...
            build_complete = utils.run_cmd_no_fail("./xmlquery BUILD_COMPLETE --value",
                                             from_dir=casedir)
    ...
    def _batch_test_fixture(self, testcase_name):
        ...
        self.run_cmd_assert_result("{}/create_newcase {}".format(self.SCRIPT_DIR, args),
    ...
    def test_cime_case_xmlchange_append(self):
        ...
        self.run_cmd_assert_result("./xmlchange --id PIO_CONFIG_OPTS --val='-opt1'", from_dir=casedir)
        result = self.run_cmd_assert_result("./xmlquery --value PIO_CONFIG_OPTS", from_dir=casedir)
        self.assertEqual(result, "-opt1")

        self.run_cmd_assert_result("./xmlchange --id PIO_CONFIG_OPTS --val='-opt2' --append", from_dir=casedir)
        result = self.run_cmd_assert_result("./xmlquery --value PIO_CONFIG_OPTS", from_dir=casedir)
    ...
    def test_cime_case_test_walltime_mgmt_1(self):
        ...
        result = self.run_cmd_assert_result("./xmlquery JOB_WALLCLOCK_TIME --subgroup=case.test --value", from_dir=casedir)
        self.assertEqual(result, "00:10:00")

        result = self.run_cmd_assert_result("./xmlquery JOB_QUEUE --subgroup=case.test --value", from_dir=casedir)
    ...
    def test_cime_case_test_walltime_mgmt_5(self):
        ...
        self.run_cmd_assert_result("./xmlchange JOB_QUEUE=slartibartfast --subgroup=case.test", from_dir=casedir, expected_stat=1)

        self.run_cmd_assert_result("./xmlchange JOB_QUEUE=slartibartfast --force --subgroup=case.test", from_dir=casedir)
    ...
    def test_cime_case_test_walltime_mgmt_6(self):
        ...
        self.run_cmd_assert_result("./xmlchange JOB_WALLCLOCK_TIME=421:32:11 --subgroup=case.test", from_dir=casedir)
    ...
    def test_cime_case_test_custom_project(self):
        ...
        result = self.run_cmd_assert_result("./xmlquery --value PROJECT --subgroup=case.test", from_dir=casedir)
    ...
    def test_configure(self):
        ...
        utils.run_cmd_no_fail("{} --machine={} --compiler={}".format(os.path.join(utils.get_cime_root(), "tools", "configure"), self._machine, self._compiler), from_dir=manual_config_dir)
    ...
    def test_self_build_cprnc(self):
        ...
        self.run_cmd_assert_result("./xmlchange CCSM_CPRNC=this_is_a_broken_cprnc",
                              from_dir=casedir)
