...
class TestCreateNewcase(base.BaseTestCase):
    ...
    def test_b_user_mods(self):
        ...
        user_mods_dir = os.path.join(utils.get_python_libs_root(), "..", "tests", "user_mods_test1")
    ...
    def test_c_create_clone_keepexe(self):
        ...
        user_mods_dir = os.path.join(utils.get_python_libs_root(), "..", "tests", "user_mods_test3")

        cmd = "%s/create_clone --clone %s --case %s --keepexe --user-mods-dir %s" \
              % (self.SCRIPT_DIR, prevtestdir, testdir, user_mods_dir)
    ...
    def test_d_create_clone_new_user(self):
        ...
        self.run_cmd_assert_result("./xmlchange USER=this_is_not_a_user",
                              from_dir=prevtestdir)

        fakeoutputroot = cls._testroot.replace(os.environ.get("USER"), "this_is_not_a_user")
        self.run_cmd_assert_result("./xmlchange CIME_OUTPUT_ROOT=%s"%fakeoutputroot,
                              from_dir=prevtestdir)

        # this test should pass (user name is replaced)
        self.run_cmd_assert_result("%s/create_clone --clone %s --case %s " %
                              (self.SCRIPT_DIR, prevtestdir, testdir),from_dir=self.SCRIPT_DIR)

        shutil.rmtree(testdir)
        # this test should pass
        self.run_cmd_assert_result("%s/create_clone --clone %s --case %s --cime-output-root %s" %
                              (self.SCRIPT_DIR, prevtestdir, testdir, cls._testroot),from_dir=self.SCRIPT_DIR)
    ...
    def test_f_createnewcase_with_user_compset(self):
        ...
        if utils.get_model() == "cesm":
            if utils.get_cime_default_driver() == "nuopc":
                pesfile = os.path.join(utils.get_src_root(),"components","cmeps","cime_config","config_pes.xml")
            else:
                pesfile = os.path.join(utils.get_src_root(),"components","cpl7","driver","cime_config","config_pes.xml")
        else:
            pesfile = os.path.join(utils.get_src_root(),"driver-mct","cime_config","config_pes.xml")
    ...
    def test_k_append_config(self):
        ...
        newmachfile = os.path.join(utils.get_cime_root(),"config",
    ...
    def test_ka_createnewcase_extra_machines_dir(self):
        ...
        newmachfile = os.path.join(utils.get_cime_root(),"config",
                                   "xml_schemas","config_machines_template.xml")
        utils.safe_copy(newmachfile, os.path.join(extra_machines_dir, "config_machines.xml"))
...
"""
        ...
        # Run case.setup
        self.run_cmd_assert_result("./case.setup",
                              from_dir=testdir)
    ...
    def test_m_createnewcase_alternate_drivers(self):
        ...
        for driver in ("nuopc", "moab"):
            if not os.path.exists(os.path.join(utils.get_cime_root(),"src","drivers",driver)):
