...
class TestMakeMacros(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.test_compiler = self.MACHINE.get_default_compiler() if self.TEST_COMPILER is None else self.TEST_COMPILER
        self.test_mpilib   = self.MACHINE.get_default_MPIlib(attributes={"compiler":self.test_compiler}) if self.TEST_MPILIB is None else self.TEST_MPILIB
    ...
    def xml_to_tester(self, xml_string):
        ...
        return test_utils.MakefileTester(self, test_utils.get_macros(self._maker, test_xml, "Makefile"))
