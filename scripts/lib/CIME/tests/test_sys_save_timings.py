...
class TestSaveTimings(base.BaseTestCase):
    def simple_test(self, manual_timing=False):
        ...
        if manual_timing:
            self.run_cmd_assert_result("cd %s && %s/save_provenance postrun" % (casedir, self.TOOLS_DIR))
    ...
    def _record_success(self, test_name, test_success, commit, exp_last_pass, exp_trans_fail, baseline_dir):
        provenance.save_test_success(baseline_dir, None, test_name, test_success, force_commit_test=commit)
        was_success, last_pass, trans_fail = provenance.get_test_success(baseline_dir, None, test_name, testing=True)
    ...
    def test_success_recording(self):
        ...
        # Test initial state
        was_success, last_pass, trans_fail = provenance.get_test_success(baseline_dir, None, fake_test1, testing=True)
