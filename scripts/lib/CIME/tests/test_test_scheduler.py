            ...
            for test in ct._tests:
                if (phase == test_scheduler.TEST_START):
                    ...
                    if (test == build_fail_test):
                        ct._update_test_status(test, phase, test_status.TEST_FAIL_STATUS)
                    ...
                    with self.assertRaises(utils.CIMEError):
                        ct._update_test_status(test, ct._phases[idx+1], test_status.TEST_PEND_STATUS)
    ...
    ###########################################################################
        ...
        for x in test_statuses:
            ...
            if (test_name == build_fail_test):
                ...
                self.assert_test_status(test_name, ts, test_status.MODEL_BUILD_PHASE, test_status.TEST_FAIL_STATUS)
                ...
                data = open(log_file, "r").read()
                self.assert_test_status(test_name, ts, test_status.SHAREDLIB_BUILD_PHASE, test_status.TEST_FAIL_STATUS)
                self.assertTrue("Exception from init" in data,
                                "Broken test did not report build error:\n%s" % data)
            elif (test_name == run_fail_test):
                self.assert_test_status(test_name, ts, test_status.RUN_PHASE, test_status.TEST_FAIL_STATUS)
                ...
                self.assert_test_status(test_name, ts, test_status.MEMLEAK_PHASE, test_status.TEST_FAIL_STATUS)
                self.assert_test_status(test_name, ts, test_status.RUN_PHASE, test_status.TEST_PASS_STATUS)
            elif (test_name == test_diff_test):
                self.assert_test_status(test_name, ts, "COMPARE_base_rest", test_status.TEST_FAIL_STATUS)
                self.assert_test_status(test_name, ts, test_status.RUN_PHASE, test_status.TEST_PASS_STATUS)
            elif test_name == st_arch_fail_test:
                self.assert_test_status(test_name, ts, test_status.RUN_PHASE, test_status.TEST_PASS_STATUS)
                self.assert_test_status(test_name, ts, test_status.STARCHIVE_PHASE, test_status.TEST_FAIL_STATUS)
            else:
        ...
        for x in test_statuses:
            ...
            if test_name == build_fail_test:
                ...
                with test_status.TestStatus(test_dir=casedir) as ts:
                    ts.set_status(test_status.MODEL_BUILD_PHASE, test_status.TEST_PEND_STATUS)
            elif test_name == run_fail_test:
                self.assert_test_status(test_name, ts, test_status.RUN_PHASE, test_status.TEST_FAIL_STATUS)
                with test_status.TestStatus(test_dir=casedir) as ts:
                    ts.set_status(test_status.SUBMIT_PHASE, test_status.TEST_PEND_STATUS)
            else:
                self.assertTrue(test_name == pass_test)
                self.assert_test_status(test_name, ts, test_status.MODEL_BUILD_PHASE, test_status.TEST_PASS_STATUS)
