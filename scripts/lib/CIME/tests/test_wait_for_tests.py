...
class TestWaitForTests(base.BaseTestCase):
    def setUp(self):
        ...
        for r in range(10):
            for testdir in basic_tests:
                ...
                test_utils.make_fake_teststatus(os.path.join(testdir, str(r)), "Test_%d" % r, test_status.TEST_PASS_STATUS, test_status.RUN_PHASE)

        test_utils.make_fake_teststatus(os.path.join(self._testdir_with_fail,   "5"), "Test_5", test_status.TEST_FAIL_STATUS, test_status.RUN_PHASE)
        test_utils.make_fake_teststatus(os.path.join(self._testdir_unfinished,  "5"), "Test_5", test_status.TEST_PEND_STATUS, test_status.RUN_PHASE)
        test_utils.make_fake_teststatus(os.path.join(self._testdir_unfinished2, "5"), "Test_5", test_status.TEST_PASS_STATUS, test_status.SUBMIT_PHASE)

        integration_tests = self._testdirs[len(basic_tests):]
        for integration_test in integration_tests:
            os.makedirs(os.path.join(integration_test, "0"))
            test_utils.make_fake_teststatus(os.path.join(integration_test, "0"), "Test_0", test_status.TEST_PASS_STATUS, test_status.CORE_PHASES[0])
    ...
    def simple_test(self, testdir, expected_results, extra_args="", build_name=None):
        ...
        output = self.run_cmd_assert_result("%s/wait_for_tests -p ACME_test */TestStatus %s" % (self.TOOLS_DIR, extra_args),
    ...
    def test_wait_for_test_wait_for_pend(self):
        ...
        with test_status.TestStatus(test_dir=os.path.join(self._testdir_unfinished, "5")) as ts:
    ...
    def test_wait_for_test_wait_for_missing_run_phase(self):
        ...
        with test_status.TestStatus(test_dir=os.path.join(self._testdir_unfinished2, "5")) as ts:
            ts.set_status(test_status.RUN_PHASE, test_status.TEST_PASS_STATUS)
    ...
    def live_test_impl(self, testdir, expected_results, last_phase, last_status):
        ...
        for core_phase in test_status.CORE_PHASES[1:]:
            with test_status.TestStatus(test_dir=os.path.join(self._testdir_teststatus1, "0")) as ts:
                ts.set_status(core_phase, last_status if core_phase == last_phase else test_status.TEST_PASS_STATUS)
    ...
    def test_wait_for_test_test_status_integration_pass(self):
        self.live_test_impl(self._testdir_teststatus1, ["PASS"], test_status.RUN_PHASE, test_status.TEST_PASS_STATUS)

    def test_wait_for_test_test_status_integration_submit_fail(self):
        self.live_test_impl(self._testdir_teststatus1, ["FAIL"], test_status.SUBMIT_PHASE, test_status.TEST_FAIL_STATUS)
