...
!===============================================================================
  ...
  subroutine seq_flux_ocnalb_mct( infodata, ocn, a2x_o, fractions_o, xao_o , dtime) !+tht dtime
    ...
    integer(in),optional    , intent(in)    :: dtime !+tht: time step of coupling interval
    ...
    real(r8)            :: dtavg                !+tht: time step of coupling interval
    !
...
!+tht added dtavg control block (repeat do loop for vector performance)
         calday = nextsw_cday
         ...
         if (present(dtime)) then 
          ...
          do n=1,nloc_o
             ...
             cosz = shr_orb_cosz( calday, rlat, rlon, delta ,dtavg)
