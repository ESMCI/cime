...
contains
  ...
  subroutine shr_pcdf_readwrite(type,iosystem,pio_iotype,filename,&
    ...
    if (readtype) then
       ...
       if(rcode .ne. PIO_NOERR) then
          call shr_sys_abort(trim(subname)//' ERROR: failed to open file '//trim(filename)//' to read.')
       endif
    else
       ...
       if (lclobber .or. .not.exists) then
          ...
          if(rcode .ne. PIO_NOERR) then
             call shr_sys_abort(trim(subname)//' ERROR: failed to create file '//trim(filename))
          endif
       else
          nmode = pio_write
          rcode = pio_openfile(iosystem, fid, pio_iotype, trim(filename), nmode)
  ...
  subroutine shr_pcdf_defvar0d(fid,fname,vtype)
    ...
    if(rcode .ne. PIO_NOERR) then
       call shr_sys_abort(trim(subname)//' ERROR: failed to define variable '//trim(fname))
    endif
    ...
    if(rcode .ne. PIO_NOERR) then
       call shr_sys_abort(trim(subname)//' ERROR: failed to define _FillValue for  '//trim(fname))
    endif
  ...
  subroutine shr_pcdf_defvar1d(fid,fname,vtype,dimid)
    ...
    if (rcode .ne. PIO_NOERR) then
       call shr_sys_abort(subname//' : failed to define variable '//trim(fname))
    endif
    if (vtype == PIO_DOUBLE) then
       rcode = PIO_put_att(fid, varid, '_FillValue', fillvalue)
    else
    ...
    if (rcode .ne. PIO_NOERR) then
       call shr_sys_abort(subname//' : failed to define variable _FillValue '//trim(fname))
    endif
  ...
  subroutine shr_pcdf_readr1d(fid,fname,iodesc,r1d)
    ...
    if (rcode .ne. PIO_NOERR) then
       call shr_sys_abort(subname//' : failed to find variable '//trim(fname))
    endif
    call pio_read_darray(fid,varid,iodesc,r1d,rcode)
    if (rcode .ne. PIO_NOERR) then
       call shr_sys_abort(subname//' : error writing variable '//trim(fname))
    endif
  ...
  subroutine shr_pcdf_readi1d(fid,fname,iodesc,i1d)
    ...
    if (rcode .ne. PIO_NOERR) then
       call shr_sys_abort(subname//' : error reading variable '//trim(fname))
    endif
  ...
  subroutine shr_pcdf_readdim(fid,fname,dim)
    ...
    if (rcode .ne. PIO_NOERR) then
       call shr_sys_abort(subname//' : failed to find dimension '//trim(fname))
    endif
  ...
  subroutine shr_pcdf_writedim(fid,fname,dim)
    ...
    if (rcode .ne. PIO_NOERR) then
       call shr_sys_abort(subname//' : failed to define dimension '//trim(fname))
    endif
