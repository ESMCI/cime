...
#define NUM_TYPES_TO_TEST 3
    ...
    if (my_rank < TARGET_NTASKS)
    {
        ...
        for (int t = 0; t < NUM_TYPES_TO_TEST; t++)
        {
            ...
            if ((ret = create_decomposition_1d(TARGET_NTASKS, my_rank, iosysid, test_type[t], &ioid)))
                return ret;

            /* Test decomposition read/write. */
            if ((ret = test_decomp_read_write(iosysid, ioid, num_flavors, flavor, my_rank, test_type[t],
                                              test_comm)))
                return ret;
