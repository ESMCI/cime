 ...
 * @returns 0 for success, error code otherwise. 
 */
int test_darray_fill(int iosysid, int num_flavors, int *flavor, int my_rank, MPI_Comm test_comm)
{
    int ioid;
    int my_test_size;
...
int main(int argc, char **argv)
{
    ...
    if (my_rank < TARGET_NTASKS)
    {
        ...
        if ((ret = test_darray_fill(iosysid, num_flavors, flavor, my_rank, test_comm)))
            return ret;
