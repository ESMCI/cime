...
def parse_command_line(args, description):
    ...
    parser.add_argument(
        "--test-id", default=DEFAULT_TESTID, help="test-id to use for all timing runs"
    )
    ...
    for c in COMPONENT_LIST:
        parser.add_argument(
            "--blocksize-%s" % c.lower(),
            help="minimum blocksize for component %s, if " "different from --blocksize",
            type=int,
        )

    parser.add_argument(
        "--total-tasks", type=int, help="Number of pes available for assignment"
    )

    parser.add_argument(
        "--layout", help="name of layout to solve (default selected internally)"
    )
    ...
    parser.add_argument(
        "--print-models", action="store_true", help="print all costs and ntasks"
    )
    ...
    if args.total_tasks is None and args.json_input is None:
        expect(
            args.total_tasks is not None or args.json_input is not None,
...
def _parse_timing_files(timing_files):
    ...
    for timing_file in timing_files:
        ...
        logger.debug(
            "ntasks: %s"
            % "; ".join(
                [str(k) + ":" + str(timing[k]["ntasks"]) for k in timing.keys()]
            )
        )
        logger.debug(
            "cost: %s"
            % "; ".join([str(k) + ":" + str(timing[k]["cost"]) for k in timing.keys()])
        )
        for key in timing:
            if key not in data:
            ...
            if timing[key]["ntasks"] in data[key]["ntasks"]:
                logger.warning(
                    "WARNING: duplicate timing run data in %s " "for %s ntasks=%d.",
                    timing_file,
                    key,
                    timing[key]["ntasks"],
                )
                ...
                logger.warning(
                    "Existing value: cost=%s. Ignoring new value: " "cost=%s",
                    data[key]["cost"][index],
                    timing[key]["cost"],
                )
                ...
                expect(
                    False,
                    "Timing files have inconsistant model components {} has {} vs {}".format(
                        key, data[key]["name"], timing[key]["name"]
                    ),
                )
            else:
...
def _read_timing_file(filename):
    ...
    for line in timing_lines:
        ...
        m = re.search(
            r"(\w+) Run Time:\s+(\d+\.\d+) seconds \s+(\d+\.\d+)" " seconds/mday", line
        )
        if m:
...
def load_balancing_solve(
    ...
    if json_input is not None:
        ...
        timing_files = _locate_timing_files(test_root, test_id, timing_dir)
        ...
        if layout is None:
            ...
            if "ATM" in data and "OCN" in data and "WAV" in data:
                ...
                oname = data["OCN"]["name"]
                wname = data["WAV"]["name"]
                if aname not in ("DATM", "XATM", "SATM") and oname not in (
                    "DOCN",
                    "XOCN",
                    "SOCN",
                ):
