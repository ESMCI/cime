...
"""
    ...
    # Required arguments
    parser.add_argument("--compset", help="Specify compset", required=True)
    parser.add_argument("--res", help="Specify resolution", required=True)
    ...
    parser.add_argument(
        "--extra-options-file", help="file listing options to be run using xmlchange"
    )
    parser.add_argument(
...
def load_balancing_submit(
        ...
        with Case(testname) as case:
            ...
            if extra_options_file is not None:
                try:
                    ...
                    expect(
                        False,
                        "ERROR: Could not read file {}".format(extra_options_file),
                    )

    tests = TestScheduler(
        test_names, use_existing=True, test_root=test_root, test_id=test_id
    )
    ...
    # need to fix
    logger.info(
        "Timing jobs submitted. After jobs completed, run to optimize "
        "pe layout:\n  load_balancing_solve --test-id {} --test-root {}".format(
            test_id, test_root
        )
    )
