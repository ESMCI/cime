...
TEST_DIR = os.path.join(SCRIPT_DIR, "..", "tools", "load_balancing_tool", "tests")
X_OPTIONS = """
STOP_N=1
"""
JSON_DICT = {
...
class LoadBalanceTests(unittest.TestCase):
    ...
    def test_read_and_write_json(self):
        ...
        with tempfile.NamedTemporaryFile(
            "w+"
        ) as jsonfile1, tempfile.NamedTemporaryFile("w+") as jsonfile2:
            json.dump(JSON_DICT, jsonfile1)
            jsonfile1.flush()
            cmd = "./load_balancing_solve.py --json-input %s --json-output %s" % (
                jsonfile1.name,
                jsonfile2.name,
            )
    ...
    def test_solve_from_timing_dir(self):
        cmd = (
            "./load_balancing_solve.py --timing-dir %s --total-tasks 64 --blocksize 2 --layout IceLndAtmOcn"
            % os.path.join(TEST_DIR, "timing")
        )
        output = run_cmd_no_fail(cmd, from_dir=CODE_DIR)
    ...
    def test_write_pes(self):
        with tempfile.NamedTemporaryFile(
            "w+"
        ) as jsonfile1, tempfile.NamedTemporaryFile("w+") as pes_file:
            json.dump(JSON_DICT, jsonfile1)
            jsonfile1.flush()
    ...
    def test_set_blocksize_atm(self):
        cmd = (
            "./load_balancing_solve.py --timing-dir %s --total-tasks 64 --blocksize 2 --blocksize-atm 4 --layout IceLndAtmOcn"
            % os.path.join(TEST_DIR, "timing")
        )
        output = run_cmd_no_fail(cmd, from_dir=CODE_DIR)
    ...
    def test_graph_models(self):
        ...
        with tempfile.NamedTemporaryFile("w+") as jsonfile:
            ...
            jsonfile.flush()
            cmd = "./load_balancing_solve.py --json-input %s --graph-models" % (
                jsonfile.name
            )
            output = run_cmd_no_fail(cmd, from_dir=CODE_DIR)
            self._check_solution(output, "NTASKS_ATM", 992)
    ...
    def test_xcase_submit(self):
        ...
        test_name = get_full_test_name(
            "PFS_I0", grid="f19_g16", compset="X", machine=machine, compiler=compiler
        )
        expected_dir = os.path.join(
            test_root, "{}.test_lbt".format(test_name), "timing"
        )
        if not os.path.isdir(expected_dir):
            with tempfile.NamedTemporaryFile(
                "w+"
            ) as tfile, tempfile.NamedTemporaryFile("w+") as xfile:
                tfile.write(PES_XML)
                tfile.flush()
                xfile.write(X_OPTIONS)
                xfile.flush()
                cmd = "./load_balancing_submit.py --pesfile {} --res f19_g16 --compset X --test-id test_lbt  --extra-options-file {} --test-root {}".format(
                    tfile.name, xfile.name, test_root
                )
                if MACHINE.has_batch_system():
                    sys.stdout.write(
                        "Jobs will be submitted to queue. Rerun "
                ...
                self.assertTrue(
                    output.find("Timing jobs submitted") >= 0,
                    "Expected 'Timing jobs submitted' in output",
                )

        if os.path.isdir(expected_dir):

            cmd = "./load_balancing_solve.py --total-tasks 32 --blocksize 1 --test-id test_lbt --print-models --test-root {} --layout IceLndAtmOcn".format(
                test_root
            )
            output = run_cmd_no_fail(cmd, from_dir=CODE_DIR)
            self.assertTrue(
                output.find("***ATM***") > 0, "--print-models failed to print ATM data"
            )
    ...
    def test_use_atm_lnd(self):
        ...
        with tempfile.NamedTemporaryFile("w+") as jsonfile1:
            ...
            jsonfile1.flush()
            cmd = (
                "./load_balancing_solve.py --json-input %s --print-models --layout tests.atm_lnd.AtmLnd"
                % (jsonfile1.name)
            )
            output = run_cmd_no_fail(cmd, from_dir=CODE_DIR)
