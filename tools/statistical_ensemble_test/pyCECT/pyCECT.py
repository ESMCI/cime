...
def main(argv):
    ...
    if (
        opts_dict["web_enabled"] == False
        and os.path.isfile(opts_dict["sumfile"]) == False
    ):
    ...
    # cam
        ...
        if opts_dict["printStdMean"]:
                ...
                for fc, avar in enumerate(alist):
                    ...
                    for i in range(comp_std_gm[f + fc].size):
                        if comp_std_gm[f + fc][i] > dist_995:
                            ...
                            comp_std_gm[f + fc][i] < dist_995
                            and comp_std_gm[f + fc][i] > dist_75
                        ):
                            p = p + 1
                        elif (
                            comp_std_gm[f + fc][i] > dist_05
            ...
            for key in sorted(category):
                ...
                if len(value) != 0:
                    ...
                    plt.xticks(
                        list(range(len(list_array))), list_var, fontsize=8, rotation=-45
                    )
        ...
        if opts_dict["saveResults"]:
            ...
            v_std_gm = nc_savefile.createVariable(
                "std_gm", "f8", ("nvars", "test_size")
            )
            v_scores = nc_savefile.createVariable(
                "scores", "f8", ("nvars", "test_size")
            )
            v_ens_sigma_scores = nc_savefile.createVariable(
                "ens_sigma_scores", "f8", ("nvars",)
            )
            v_ens_std_gm = nc_savefile.createVariable(
                "ens_std_gm", "f8", ("nvars", "ens_size")
            )
