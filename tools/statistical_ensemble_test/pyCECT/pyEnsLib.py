...
def pop_zpdf(
    ...
    if test_failure:
        ...
        Zpr = np.where(Zscore_nomask <= opts_dict["pop_tol"])[0].size / float(
            Zscore_temp.count()
        )
        return Zpr
...
def search_sumfile(opts_dict, ifiles):
    ...
    if "testtype" in global_att:
        ...
        print(
            "ERROR: No global attribute testtype in your validation file => EXITING...."
        )
        sys.exit(2)
    ...
    if "model_version" in global_att:
        ...
        print(
            "ERROR: No global attribute model_version in your validation file => EXITING...."
        )
        sys.exit(2)
        ...
        for i in os.listdir(sumfile_dir):
            if os.path.isfile(sumfile_dir + i):
                ...
                if (
                    ...
                    and getsttr(sumfile_id, "compset") == compset
                ):
...
def pre_PCA(gm_orig, all_var_names, whole_list, me):
    ...
    for var in range(nvar):
        for file in range(nfile):
            if np.any(sigma_gm[var] == 0.0) and all_var_names[var] not in set(
                whole_list
            ):
    ...
    if not b_exit:
        ...
        for var in range(nvar):
            for file in range(nfile):
                standardized_global_mean[var, file] = (
                    gm[var, file] - mu_gm[var]
                ) / sigma_gm[var]
        ...
        dep_var_list = get_dependent_vars_index(
            standardized_global_mean, standardized_rank
        )
...
def calc_nrmse(orig_array, comp_array):
    ...
    sumsqr = np.sum(
        np.square(orig_array.astype(np.float64) - comp_array.astype(np.float64))
    )
...
def get_area_wgt(o_files, is_SE, nlev, popens):
    ...
    if is_SE == True:
        ...
        area_wgt = np.zeros(
            nlat, dtype=np.float64
        )  # note gauss weights are length nlat
        area_wgt[:] = gw[:]
...
def generate_global_mean_for_summary(
    o_files, var_name3d, var_name2d, is_SE, pepsi_gm, opts_dict
):
    ...
    for count, vname in enumerate(var_name3d):
        ...
        if np.any(np.isnan(data)):
            print(
                "ERROR: ",
                vname_d,
                " data contains NaNs - please check input => EXITING",
            )
            nan_flag = True
            continue
        if is_SE == True:
...
def read_ensemble_summary(ens_file):
    ...
    for k, v in fens.variables.items():
        ...
        # Retrieve the RMSZ score array
            ...
            loadings_gm = np.zeros(
                (num_var3d + num_var2d, num_var3d + num_var2d), dtype=np.float64
            )
            loadings_gm[:, :] = v[:, :]
...
#
def standardized(
    gm, mu_gm, sigma_gm, loadings_gm, all_var_names, opts_dict, ens_avg, me
):
    nvar = gm.shape[0]
    nfile = gm.shape[1]
                ...
                if thefile in temp0[key]:
                    ...
                    if temp < 1:
                        ...
                        print(
                            k
                            + "       ("
                            + "{0:9.2e}".format(v[name][thefile])
                            + " outside of ["
                            + "{0:9.2e}".format(variables[k][namerange][0])
                            + " "
                            + "{0:9.2e}".format(variables[k][namerange][1])
                            + "])"
                        )
...
def comparePCAscores(ifiles, new_scores, sigma_scores_gm, opts_dict, me):
    ...
    for i in range(opts_dict["nPC"]):
        ...
        if len(index_list) > 0 and me.get_rank() == 0:
            print(
                "PC " + str(i + 1) + ": failed " + str(len(index_list)) + " runs ",
                index_list,
            )
    if me.get_rank() == 0:
        print(" ")
    ...
    if opts_dict["eet"] >= opts_dict["numRunFile"]:
        ...
        passes, failures = eet.test_combinations(
            faildict,
            runsPerTest=opts_dict["numRunFile"],
            nRunFails=opts_dict["minRunFail"],
        )
        if me.get_rank() == 0:
            print(" ")
            print(
                "%d tests failed out of %d possible tests."
...
def CECT_usage():
    ...
    print(
        "   --sumfile  <ifile>      : the ensemble summary file (generated by pyEnsSum.py)"
    )
    print(
        "   --indir    <path>       : directory containing the input run files (at least 3 files)"
    )
    print(
        "   --tslice   <num>        : which time slice to use from input run files (default = 1)"
    )
    print("  ----------------------------")
    ...
    print(
        "   --numRunFile <num>      : total number of runs to include in test (default = 3)"
    )
    print(
    ...
    print("  ----------------------------")
    print(
        "   --popens                : indicate POP-ECT (required!) (tslice will bet set to 0)"
    )
    print(
        "   --jsonfile  <file>      : list the json file that specifies variables to test (required!), e.g. pop_ensemble.json"
    )
    print(
        "   --pop_tol <num>         : set pop zscore tolerance (default is 3.0 - recommended)"
    )
...
def get_files_from_glob(opts_dict):
    ...
    for fname in in_files:
        istr = fname.find(".nc")
        temp = (
            (int(fname[istr - 7 : istr - 3]) - 1) * 12 + int(fname[istr - 2 : istr]) - 1
        )
...
def pop_compare_raw_score(opts_dict, ifiles, timeslice, Var3d, Var2d):
    ...
    Zscore_tran = np.zeros(
        (len(ifiles), len(Var3d) + len(Var2d), (nbin)), dtype=np.float32
    )
    ...
    for fcount, fid in enumerate(ifiles):
        ...
        print(
            ...
            + str(fcount + 1)
            + " (file="
            + in_file_names[fcount]
            + "):"
        )

        if timeslice >= ens_timeslice:
            print(
                "WARNING: The summary file containing only ",
                ens_timeslice,
                " timeslices. Skipping this run evaluation...",
            )
            continue
